...
export default function GeoAdventure6Vhodno({ initialMode = null, exitHref = "/igri" }) {
  ...
  const loadScramble = (idx) => {
    const item = GEO_6_VHODNO_DATA.scramble[idx];
    if (!item) {
      endGame(true, "Браво! Подреди всички разбъркани географски термини!");
      return;
    }
  ...
  const startHangman = () => {
    setHangmanItem(pickRandom(GEO_6_VHODNO_DATA.hangman));
    setGuessed([]);
    setLives(5);
    setDisabledLetters([]);
    setMode("hangman");
  ...
  const startMatch = () => {
    ...
    setMatchDefs(shuffleArray(GEO_6_VHODNO_DATA.match));
    setMode("match");
    setResult(null);
  };
  ...
  const handleGuess = (letter) => {
    ...
    if (hangmanItem.word.includes(letter)) {
      ...
      if (won) endGame(true, "Позна успешно термина!");
    } else {
  ...
  const checkScramble = () => {
    const item = GEO_6_VHODNO_DATA.scramble[scrambleIndex];
    if (!item) return;
  ...
  const pickGroup = (group) => {
    const item = GEO_6_VHODNO_DATA.sort[sortIndex];
    if (!item) return;
    if (group === item.group) {
      const next = sortIndex + 1;
      if (next >= GEO_6_VHODNO_DATA.sort.length) {
        endGame(true, "Отлично! Сортира всички понятия правилно!");
        return;
      }
      setSortIndex(next);
      setWrongGroup(null);
  ...
  // Odd one logic
  const pickOdd = (opt) => {
    const item = GEO_6_VHODNO_DATA.oddOne[oddIndex];
    if (!item) return;
    if (opt === item.correct) {
      const next = oddIndex + 1;
      if (next >= GEO_6_VHODNO_DATA.oddOne.length) {
  ...
  const selectMatch = (type, value) => {
    ...
    if (term === def.term) {
      ...
      setMatchSelectedTerm(null);
      setMatchSelectedDef(null);
      if (nextMatched.length >= GEO_6_VHODNO_DATA.match.length) {
        setTimeout(() => {
      ...
      <div className={styles.shell}>
        <div className={styles.result}>
          ...
          <p className={styles.resultMsg}>
            {result.success ? result.message : `Търсената дума беше: ${result.message}`}
          </p>
  ...
  if (!mode) {
    ...
    return (
      <div className={styles.shell}>
        ...
        <p className={styles.footnote}>По темите от входното ниво по география за 6. клас</p>
      </div>
    );
  }
          ...
          {CYRILLIC_ALPHABET.map((letter) => {
            ...
            return (
              <button
                ...
                type="button"
                disabled={used}
                className={[styles.key, correct ? styles.keyOk : "", wrong ? styles.keyBad : ""]
                  .filter(Boolean)
                  .join(" ")}
      ...
      <div className={styles.shell}>
        <div className={styles.topBar}>
          ...
          <div className={styles.progressTeal}>
            Ниво: {scrambleIndex + 1} / {GEO_6_VHODNO_DATA.scramble.length}
          </div>
        </div>
      ...
      <div className={styles.shell}>
        <div className={styles.topBar}>
          ...
          <div className={styles.progressAmber}>
            Прогрес: {sortIndex + 1}/{GEO_6_VHODNO_DATA.sort.length}
          </div>
        </div>
        <p className={styles.label}>Към коя група принадлежи понятието?</p>
        <div className={styles.sortTarget}>{item.item}</div>
        <div className={styles.continentGrid}>
          {GEO_6_VHODNO_DATA.sortGroups.map((g) => (
            <button
              key={g}
              type="button"
              className={`${styles.continentBtn} ${wrongGroup === g ? styles.flashBad : ""}`}
              onClick={() => pickGroup(g)}
            >
              {g}
            </button>
      ...
      <div className={styles.shell}>
        <div className={styles.topBar}>
          ...
          <div className={styles.progressPurple}>
            Въпрос: {oddIndex + 1}/{GEO_6_VHODNO_DATA.oddOne.length}
          </div>
        </div>
      ...
      <div className={styles.shell}>
        ...
        <div className={styles.matchGrid}>
          <div className={styles.matchCol}>
            {GEO_6_VHODNO_DATA.match.map((item) => {
